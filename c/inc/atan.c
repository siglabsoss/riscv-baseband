 ...
 * integer (see C99 standard 7.20.6.1.2 and foot/note 265 for the description of
...
uint16_t fxpt_atan2(const int16_t y, const int16_t x) {
    ...
    if (nabs_x < nabs_y) { // octants 1, 4, 5, 8
        ...
        const int16_t correction = q15_mul(2849, s16_nabs(y_over_x));
        const int16_t unrotated = q15_mul(11041 + correction, y_over_x);
        if (x > 0) { // octants 1, 8
            return unrotated;
        } else { // octants 4, 5
        ...
        const int16_t correction = q15_mul(2849, s16_nabs(x_over_y));
        const int16_t unrotated = q15_mul(11041 + correction, x_over_y);
        if (y > 0) { // octants 2, 3
